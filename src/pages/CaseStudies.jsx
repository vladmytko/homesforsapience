        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-20 gap-y-10">
          {caseStudiesList.map((item) => (
            <Link
              ...
              className="group bg-transparent flex flex-col w-100 h-75 justify-self-center transform-gpu transition duration-500 ease-out hover:scale-[1.02]"
            >
              <div className="relative w-full h-100 rounded-xl overflow-hidden shadow-xl transition duration-500 ease-out group-hover:shadow-2xl">
                <img
                  src={item.image}
                  alt={item.title}
                ...
                <div className="absolute  bottom-0 p-5 text-white flex flex-col gap-3 transition duration-500 ease-out group-hover:-translate-y-2">
                  <h3 className="text-2xl font-semibold drop-shadow-[0_4px_12px_rgba(0,0,0,0.6)]">{item.title}</h3>
